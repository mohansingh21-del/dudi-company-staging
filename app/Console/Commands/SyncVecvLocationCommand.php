...
class SyncVecvLocationCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'vecv:sync-location
     ...
     * @var string
     */
    protected $description = 'Pull live location and odometer telemetry from the VECV rFMS API and store it as readings';
     ...
     * @return int
     */
    public function handle(VecvLocationSyncService $sync)
    {
        $chassis = [];
        ...
        try {
            ...
            if ($e->isRateLimited()) {
                ...
                Log::info('vecv:sync-location rate limited', ['status' => $e->getStatusCode()]);

                return 0;
            }

            $this->error($e->getMessage());
            Log::error('vecv:sync-location failed', [
        ...
        if ($summary['stale'] > 0) {
            $this->warn(sprintf(
                '%d vehicle(s) reported data older than %d minutes - their position is not live.',
                $summary['stale'],
        ...
        Log::info('vecv:sync-location completed', $summary);
