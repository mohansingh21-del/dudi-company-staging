...
*/
    ...
    if ($viewType === 'monthly') {
        ...
        $endDate = $statsDate->copy()->endOfDay();
           ...
           $employeeQuery->whereHas('attendanceProcesseds', function ($q) use (
    $viewType,
    $startDate,
    $endDate,
    $statsDate,
...
$employeeQuery = \App\Models\Employee::query()->whereDate('joining_date', '<=', $endDate->format('Y-m-d'));
    ...
    ->whereDate('from_date', '<=', $statsDate->format('Y-m-d'))
        ...
        $statsQuery->whereHas('employee', function ($q) use ($request, $statsDate, $viewType, $applyActiveFilter,
        $filterActive, $attendanceStatusInput, $endDate) {
        $q->whereDate('joining_date', '<=', $endDate->format('Y-m-d'));
            ...
            ->whereDate('joining_date', '<=', $endDate->format('Y-m-d'));
                ...
                if ($attendanceStatusInput) {
                $employeeQuery->whereHas('attendanceProcesseds', function ($q) use ($month, $year,
                ...
                $data = collect($employees->items())->map(function ($employee) use ($attendanceCounts, $leaveSummary,
                $generalHolidays, $siteHolidays, $daysInMonth) {
                $att = $attendanceCounts->get($employee->id);
                $empLeave = $leaveSummary[$employee->id] ?? ['paid' => 0, 'unpaid' => 0];
